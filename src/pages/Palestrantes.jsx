...
import { BriefcaseIcon, CaretDownIcon, MagnifyingGlassIcon, MicrophoneStageIcon } from "@phosphor-icons/react";
...
const Palestrantes = () => {
    ...
    const palestrantesFiltrados = (palestrantes || [])
        .filter(p =>
            ...
            p.formacao?.toLowerCase().includes(busca.toLowerCase())
        )
        .sort((a, b) => {
                    ...
                    <motion.div variants={itemVariants} className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-5">
                        <div>
                            <div className="inline-flex items-center gap-2 bg-accent/25 rounded-full px-4 py-2 font-secondary text-xs font-semibold text-primary mb-3">
                                ...
                                Corpo de palestrantes
                            </div>
                            <h1 className="text-4xl font-primary text-primary font-bold">Palestrantes</h1>
                            <p className="text-sm font-secondary text-primary/55 mt-2">
                                Encontre especialistas por área, instituição e formação.
                            </p>
                        </div>

                        <div className="grid grid-cols-2 gap-3 w-full xl:w-auto">
                            <MetricCard icon={<MicrophoneStageIcon size={20} />} label="Exibindo" value={palestrantesFiltrados.length} />
                            <MetricCard icon={<BriefcaseIcon size={20} />} label="Total" value={(palestrantes || []).length} />
                        </div>
                    </motion.div>
                        ...
                        <div className="flex flex-col lg:flex-row gap-3">
                            ...
                            <div className="flex items-center gap-3 flex-1 bg-base-100/65 border border-accent/10 rounded-2xl px-5 py-3.5 focus-within:ring-2 focus-within:ring-accent/50 transition-all">
                                <input id="busca" type="text" value={busca} onChange={(e) => setBusca(e.target.value)}
                                    placeholder="Buscar por nome, e-mail, CPF, área, instituição ou formação" className="flex-1 bg-transparent text-sm font-secondary text-primary placeholder:text-primary/35 focus:outline-none"/>
